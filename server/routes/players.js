...
router.get('/:user_id/stats/:league_id', auth, async (req, res) => {
  ...
  try {
    ...
    const gamesRes = await db.execute({
      sql: `SELECT g.id, g.played_at, g.note, g.winner_score, g.loser_score,
      ...
      args: [user_id, user_id, user_id, user_id, user_id, league_id],
    })
...
router.get('/h2h/:user1_id/:user2_id/:league_id', auth, async (req, res) => {
  ...
  try {
    const result = await db.execute({
      sql: `SELECT g.id, g.played_at, g.winner_id, g.loser_id, g.winner_score, g.loser_score,
              w.full_name as winner_name, l.full_name as loser_name
            FROM games g
            JOIN users w ON w.id = g.winner_id
            JOIN users l ON l.id = g.loser_id
            WHERE g.league_id = ?
